...
// 1) `npx anvil init` will set admin principal
...
shared ({ caller = _installer }) actor class Class() : async IF.Interface = this {
  ...
  // EDITABLE
  private let MAX_TOKENS = 1080; // max amount of NFTs the contract can hold
  private let MAX_CODES = 2000; // an arbitrary amount of codes
  private stable var LEFT_AIRDROP : Nat = 550; // max amount for Airdrop
  private stable var LEFT_PURCHASE : Nat = 530; // max amount for purchase
  ...
  public shared ({ caller }) func claim(aid : Nft.AccountIdentifier, subaccount : ?Nft.SubAccount, tid : Nft.TokenIdentifier) : async Result.Result<(), Text> {
    ...
    switch (r.tokens.indexOf(tid)) {
      case (?idx) {
        switch (await Cluster.nftFromTid(anvil.conf, tid).transfer({ from = #address(getScriptAccount()); to = #address(aid); token = tid; memo = Blob.fromArray([]); subaccount = null })) {
  ...
  public shared ({ caller }) func buy_tx(amount : Nat, tx_id : Nft.TransactionId, subaccount : ?Nft.SubAccount) : async Result.Result<Basket, Text> {
  ...
  public shared ({ caller }) func icp_balance() : async Result.Result<Nft.Balance, Text> {
    ...
    let res = await Cluster.pwrFromAid(anvil.conf, scriptAccount).balance({
